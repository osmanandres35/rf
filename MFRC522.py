...
class MFRC522:
    ...
    def Write_MFRC522(self, addr, val):
        self.spi.writebytes((addr << 0x01 & 0x7E, val))

    def Read_MFRC522(self, addr):
        val = self.spi.xfer2((addr << 0x01 & 0x7E | 0x80, 0x00))
        return val[0x01]
    ...
    def MFRC522_ToCard(self, command, sendData):
        ...
        while True:
            ...
            i = i - 0x01
            if ~(i != 0x00 and ~(n & 0x01) and ~(n & waitIRq)):
                break
    ...
    # Check if an error occurred
        ...
        if DEBUG:
            if not status == self.MI_OK:
                print 'AUTH ERROR!!'
            if not self.Read_MFRC522(self.Status2Reg) & 0x08 != 0x00:
    ...
    def MFRC522_Read(self, blockAddr):
        ...
        recvData.append(pOut[0x00])
        recvData.append(pOut[0x01])
        (status, backData, backLen) = \
            self.MFRC522_ToCard(self.PCD_TRANSCEIVE, recvData)
        if not status == self.MI_OK:
            print 'Error while reading!'
        i = 0x00
        if len(backData) == 0x10:
    ...
    def MFRC522_Write(self, blockAddr, writeData):
        ...
        buff.append(crc[0x00])
        buff.append(crc[0x01])
        (status, backData, backLen) = \
            self.MFRC522_ToCard(self.PCD_TRANSCEIVE, buff)
        if not status == self.MI_OK or not backLen == 0x04 \
           or not backData[0x00] & 0x0F == 0x0A:
            status = self.MI_ERR

        print '%s backdata &0x0F == 0x0A %s' % \
              (backLen, backData[0x00] & 0x0F)
        if status == self.MI_OK:
            i = 0x00
            buf = []
            while i < 0x10:
    ...
    def MFRC522_SelectTagSN(self):
        ...
        if self.MFRC522_PcdSelect1(uid) == 0x00:
            return (self.MI_ERR, [])
        if DEBUG:
        ...
        if uid[0x00] == 0x88:
            ...
            valid_uid.extend(uid[0x01:0x04])
            ...
            if uid[0x00] == 0x88:
                ...
                if self.MFRC522_PcdSelect3(uid) == 0x00:
                    return (self.MI_ERR, [])
                if DEBUG:
                    print 'PcdSelect3() {}'.format(uid)
        valid_uid.extend(uid[0x00:0x04])
